...
const els = {
  ...
  viralContent: document.querySelector("#viralContent"),
  viralAudience: document.querySelector("#viralAudience"),
  viralChannel: document.querySelector("#viralChannel"),
  viralTargetUsers: document.querySelector("#viralTargetUsers"),
  copyViralPost: document.querySelector("#copyViralPost"),
  copyLaunchEmail: document.querySelector("#copyLaunchEmail"),
  downloadViralKit: document.querySelector("#downloadViralKit"),
  viralDemo: document.querySelector("#viralDemo"),
...
async function runCommandDeckAction(command) {
  ...
  switch (command) {
    ...
    case "viral":
      renderViralLaunch();
      scrollToSection("#viralLaunch");
      showToast(state.pipeline.length ? "Viral Launch Engine opened" : "Load demo or convert CVs first");
      break;
    default:
...
function viralMeta() {
  return {
    audience: String(els.viralAudience?.value || "Recruitment agencies"),
    channel: String(els.viralChannel?.value || "LinkedIn"),
    targetUsers: Math.max(10, Number.parseInt(els.viralTargetUsers?.value || "100", 10) || 100),
  };
}

function viralMetrics() {
  const meta = viralMeta();
  const founder = founderMetrics();
  const featureCount = 10;
  const launchScore = Math.min(
    99,
    Math.round(54 + Math.min(18, founder.profiles * 4) + Math.min(14, founder.shortlist * 3) + Math.min(13, founder.projectedMrr / 20)),
  );
  const referralTarget = Math.max(meta.targetUsers, Math.round(meta.targetUsers * 1.4));
  const proof = founder.profiles
    ? `${founder.profiles} demo profiles moved through Excel, portal, interview, close, and launch reporting`
    : "demo profiles can move through Excel, portal, interview, close, and launch reporting";
  const hook =
    meta.audience === "Office assistants"
      ? "Turn a folder of CV PDFs into client-ready Excel profiles before the next coffee cools."
      : meta.audience === "HR teams"
        ? "Stop rebuilding candidate profile packs by hand when the browser can do the boring work."
        : meta.audience === "Staffing founders"
          ? "A mini operating system for small staffing teams that starts with the ugliest admin job: CV formatting."
          : "Recruitment agencies should not spend billable hours copying CV data into profile templates.";
  return { ...meta, ...founder, featureCount, launchScore, referralTarget, proof, hook };
}

function viralAngles(metrics = viralMetrics()) {
  return [
    {
      title: "Painkiller",
      body: `${metrics.audience} lose hours turning CV PDFs into client profile formats. ProfileForge makes the first result an Excel-ready pack.`,
    },
    {
      title: "Privacy",
      body: "The static browser version processes CV PDFs locally, which makes the first public demo easier for cautious teams to try.",
    },
    {
      title: "Workflow",
      body: "It goes beyond conversion: QA, combined workbook, portal, submission, interview, closing, theater, founder metrics, and launch copy.",
    },
    {
      title: "Referral",
      body: `Invite ${metrics.targetUsers} early users, ask each to share one before/after screenshot, and reward the first ${metrics.referralTarget} trial users with founding pricing.`,
    },
  ];
}

function viralPostText() {
  const metrics = viralMetrics();
  return [
    "I am launching ProfileForge.",
    "",
    metrics.hook,
    "",
    "What it does:",
    ...
    "- Export print-ready Excel profile workbooks",
    "- Build client portals, submission copy, interview kits, closing packs, and launch reports",
    "",
    `Launch proof: ${metrics.proof}.`,
    `Audience: ${metrics.audience}`,
    `First target: ${metrics.targetUsers} users`,
    "",
    "If your team formats CV profiles for clients, I would love feedback from real recruitment workflows.",
  ].join("\n");
}
...
function launchEmailText() {
  const metrics = viralMetrics();
  return [
    `Subject: Early access to ProfileForge for ${metrics.audience.toLowerCase()}`,
    "",
    "Hi,",
    "",
    "I am opening early access to ProfileForge, a browser-based tool for turning PDF CVs into client-ready Excel profile workbooks.",
    "",
    metrics.hook,
    "",
    "The current version includes field review, combined workbooks, custom templates, quality checks, client portal, submission copy, interview kit, closing pack, and launch dashboard.",
    "",
    `I am looking for the first ${metrics.targetUsers} users to test it on real CV batches and tell me what should be improved next.`,
    "",
...
function viralKitHtml() {
  const metrics = viralMetrics();
  const angles = viralAngles(metrics);
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>ProfileForge Viral Launch Kit</title>
<style>
*{box-sizing:border-box}body{margin:0;font-family:Inter,Segoe UI,Arial,sans-serif;background:#07111f;color:#17202a}.wrap{max-width:1180px;margin:auto;padding:30px}.hero,.card,.sprint{border:1px solid rgba(255,255,255,.18);border-radius:22px;background:linear-gradient(135deg,#fff,#edf8ff 35%,#fff0f7 68%,#edfff7);box-shadow:0 28px 80px rgba(0,0,0,.26)}.hero{padding:34px}.kicker{color:#214f8f;font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:.08em}h1{margin:8px 0;font-size:46px;line-height:1.04}.muted{color:#5c6876}.stats,.angles{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:16px 0}.card{padding:18px}.card span{display:block;color:#5c6876;font-size:12px;font-weight:900;text-transform:uppercase}.card b{display:block;margin-top:8px;font-size:32px}.angles{grid-template-columns:repeat(2,1fr)}.angles .card b{font-size:22px}.sprint{padding:22px}.sprint ol{display:grid;grid-template-columns:repeat(7,1fr);gap:8px;margin:0;padding:0;list-style:none}.sprint li{min-height:92px;padding:12px;border:1px solid #d6e1ec;border-radius:14px;background:#fff}.footer{margin-top:18px;color:#cbd5e1;text-align:center}@media(max-width:900px){.stats,.angles,.sprint ol{grid-template-columns:1fr}h1{font-size:34px}}
</style>
</head>
<body>
<main class="wrap">
  <section class="hero">
    <span class="kicker">ProfileForge Viral Launch Kit</span>
    <h1>${escapeHtml(metrics.hook)}</h1>
    <p class="muted">Launch channel: ${escapeHtml(metrics.channel)}. Audience: ${escapeHtml(metrics.audience)}. Target first users: ${metrics.targetUsers}.</p>
  </section>
  <section class="stats">
    <article class="card"><span>Launch score</span><b>${metrics.launchScore}</b></article>
    <article class="card"><span>Features</span><b>${metrics.featureCount}</b></article>
    <article class="card"><span>Proof</span><b>${metrics.profiles}</b></article>
    <article class="card"><span>Projected MRR</span><b>${formatUsd(metrics.projectedMrr)}</b></article>
  </section>
  <section class="angles">
    ${angles.map((angle) => `<article class="card"><span>Angle</span><b>${escapeHtml(angle.title)}</b><p class="muted">${escapeHtml(angle.body)}</p></article>`).join("")}
  </section>
  <section class="sprint">
    <span class="kicker">7-day sprint</span>
    <h2>Launch without waiting for perfection</h2>
    <ol>
      <li>Publish domain and one demo video.</li>
      <li>Post the before/after CV workflow.</li>
      <li>Message 25 recruiters directly.</li>
      <li>Collect 5 template requests.</li>
      <li>Turn feedback into one visible update.</li>
      <li>Ask users to share one screenshot.</li>
      <li>Announce founding pricing.</li>
    </ol>
  </section>
  <p class="footer">Generated by ProfileForge Viral Launch Engine</p>
</main>
</body>
</html>`;
}

function renderViralLaunch() {
  if (!els.viralContent) return;
  const metrics = viralMetrics();
  if (!metrics.profiles) {
    els.viralContent.innerHTML = '<div class="empty-state">Load demo or convert CVs to build a launch campaign.</div>';
    if (els.copyViralPost) els.copyViralPost.disabled = true;
    if (els.copyLaunchEmail) els.copyLaunchEmail.disabled = true;
    if (els.downloadViralKit) els.downloadViralKit.disabled = true;
    return;
  }

  if (els.copyViralPost) els.copyViralPost.disabled = false;
  if (els.copyLaunchEmail) els.copyLaunchEmail.disabled = false;
  if (els.downloadViralKit) els.downloadViralKit.disabled = false;
  const angles = viralAngles(metrics);
  els.viralContent.innerHTML = `
    <div class="viral-dashboard">
      <article><span>Launch score</span><strong>${metrics.launchScore}</strong><small>${escapeHtml(metrics.channel)} ready</small></article>
      <article><span>Target users</span><strong>${metrics.targetUsers}</strong><small>${escapeHtml(metrics.audience)}</small></article>
      <article><span>Share proof</span><strong>${metrics.profiles}</strong><small>workflow profiles</small></article>
      <article><span>Referral loop</span><strong>${metrics.referralTarget}</strong><small>founding price target</small></article>
    </div>
    <div class="viral-layout">
      <section class="viral-hook-card">
        <span class="launch-kicker">Launch hook</span>
        <h3>${escapeHtml(metrics.hook)}</h3>
        <p>${escapeHtml(metrics.proof)}. Use this as the lead story for ${escapeHtml(metrics.channel)}.</p>
        <div class="viral-action-row">
          <span>Before: manual CV formatting</span>
          <span>After: client-ready profile workflow</span>
          <span>Ask: feedback from real batches</span>
        </div>
      </section>
      <aside class="viral-sprint-card">
        <h3>7-Day Sprint</h3>
        <ol>
          <li>Publish domain and demo video</li>
          <li>Post before/after workflow</li>
          <li>Message 25 recruiters</li>
          <li>Collect template requests</li>
          <li>Ship one visible update</li>
          <li>Ask for screenshot shares</li>
          <li>Announce founding pricing</li>
        </ol>
      </aside>
    </div>
    <div class="viral-angles">
      ${angles
        .map(
          (angle) => `
            <article>
...
function productBriefText() {
  return [
    ...
    "- Use Viral Launch Engine to create launch hooks, social copy, email copy, referral loop, and campaign kit.",
    "",
...
function syncStickyNavigation() {
  const headerHeight = Math.ceil(els.appHeader?.getBoundingClientRect?.().height || 96);
  const navHeight = Math.ceil(els.appTabs?.getBoundingClientRect?.().height || 64);
...
els.copyViralPost?.addEventListener("click", async () => {
  await copyText(viralPostText());
  showToast("Viral social post copied");
});
els.copyLaunchEmail?.addEventListener("click", async () => {
  await copyText(launchEmailText());
  showToast("Launch email copied");
});
els.downloadViralKit?.addEventListener("click", () => {
  if (!state.pipeline.length) return;
  downloadTextFile("profileforge-viral-launch-kit.html", viralKitHtml(), "text/html");
  showToast("Viral launch kit downloaded");
});
els.viralDemo?.addEventListener("click", async () => {
  await loadDemoBatch();
  scrollToSection("#viralLaunch");
});
...
[els.viralAudience, els.viralChannel, els.viralTargetUsers].forEach((input) => {
  input?.addEventListener("input", renderViralLaunch);
  input?.addEventListener("change", renderViralLaunch);
});
...
window.addEventListener("load", syncStickyNavigation);
window.addEventListener("scroll", syncBackToTop, { passive: true });
window.addEventListener("resize", syncStickyNavigation);
